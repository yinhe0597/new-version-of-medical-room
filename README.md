...
> SQLite 与自动化门禁已通过，真实 MySQL `fresh/history/retry` 验收仍需配置
> 本机 login-path 后执行。详见
...
### Alembic 历史迁移链修复（2026-07-11，待 MySQL 实机验收）
...
- 后端全量测试 `89/89`、迁移定向测试 `19/19`、Python 编译、前端生产构建及差异检查均通过。

> 真实 MySQL 8.0.26 `fresh/history/retry` 尚未执行：当前环境未配置可用 login-path，安全 probe 已在创建任何临时资源前退出。详见
> [Alembic 迁移续接记录](docs/开发日志-2026-07-11-Alembic迁移续接记录.md)。
