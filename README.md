...
> ⚠️ **本版本功能仅存在于 `dev` 分支，`main` 分支不包含此版本**

**数据分析看板 & 大厅预约系统 & 大屏叫号动画 & 医生自主在岗：**
- 📊 **数据分析看板**：后端 8 个分析 API（概览/就诊趋势/营收趋势/医生工作量/疾病分布/药品消耗/患者类型/时段热力图），前端 ECharts 可视化（柱状图/折线图/饼图/热力图），管理员/财务/护士均可访问
- 🏥 **大厅预约系统**：新增 `lobby` 角色，独立预约工作台 + 大厅投屏界面，支持患者搜索（脱敏）、临时患者建档、预约医生、一键取号
- 🔔 **大屏叫号动画**：医生叫号时弹出全屏动画（铃铛摇晃→叫号数字→患者姓名→诊位），6 秒自动消失；Web Speech API TTS 语音播报 + Web Audio API 叮咚提示音
- 👨‍⚕️ **医生自主在岗**：医生端顶栏新增在岗/休息切换开关，替代排班表控制大厅叫号显示状态
- 📢 **通知栏**：页面顶部滚动通知条，管理员可配置内容、颜色、滚动速度
- 🎨 **大厅界面美化**：深色渐变背景 + 毛玻璃效果 + 脉冲动画 + 医生状态灯

> 📝 详细变更：[开发日志-open0.0.18-dev.md](docs/开发日志-open0.0.18-dev.md)

---

### 🏷️ open0.0.18（2026-06-15）🀄💊📋

> ⚠️ **本版本为中医诊疗功能发布，`dev` 分支专属**

**中医诊疗系统 — 数据库扩展 & 后端API & 前端界面：**
- 🀄 **数据库扩展**：Drug 表新增 13 个中药字段（is_herb~storage_condition），Visit 表 +3 字段（tcm_enabled/tcm_syndrome/tcm_diagnosis_desc），PrescriptionItem 表 +5 字段，新增 6 张关联表（tcm_diagnosis/classic_prescription_template/template_detail/decoction_usage/herb_inventory_log/herb_loss_record/herb_damage_record）
- 🩺 **中医四诊录入**：望诊（神/舌质/舌苔）、问诊（十问歌）、切诊（脉象）、辨证结论，自动生成中医术语描述
- 💊 **中药开方**：搜索中药饮片（名称/拼音/编码），设置剂量(g)和特殊煎法（先煎/后下/包煎/烊化/冲服等），自动计算费用
- 📋 **经典方剂模板**：预置 8 个经典方剂（四君子汤/六味地黄丸/逍遥散/四物汤/桂枝汤/小柴胡汤/补中益气汤/银翘散），支持搜索、一键载入、加减修改
- 🔥 **煎药用法**：总剂数、加水量、煎煮时间、服用方法、频次、禁忌
- 💊 **护士中药库存**：中药饮片库存列表、分类/状态筛选、GB/T 31774 编码、损耗登记（自动扣减）、霉变报损申请
- 📋 **管理端方剂管理**：经典汤剂模板的新增、编辑、启用/禁用管理
- 📋 **管理端报损审核**：审核中药霉变报损申请，通过后自动扣减库存
- 🐛 **代码审查修复**：补全 6 项关键问题（库存校验/扣减/异常处理/重复审核校验/前端数据提交/权限开放）

> 📝 详细变更：[开发日志-open0.0.18.md](docs/开发日志-open0.0.18.md)
