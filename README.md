...
**缺陷修复：**
- 修复出库报表和统计报表时间显示（UTC→CST时区转换）
- 护士端时间显示修复（库存变动记录、收费确认）
...
**文档更新：**
- 新增开发日志：`docs/开发日志-2026-05-07-v18.50.md`
...
**新增功能：**
- 医生端：新增"特殊计量与用法备注"栏，位于处方表格下方、诊查费上方，用于标注特殊配药要求（如静滴溶媒配比等），护士端醒目展示
- 护士端：处方详情页和打印回执均展示特殊配药备注（橙色高亮）

**部署优化：**
- 新增 `run_prod.py` 生产部署入口，数据库路径改为相对路径，支持文件夹整体迁移
- 新增 `medical_room.spec` PyInstaller 打包配置，一键生成单文件 exe
- 修复 `admin.py` 备份功能路径解析，改为从数据库配置 URI 中提取
- `config.py` 新增 `_default_db_uri()` 适配打包环境
- `__init__.py` 静态文件路径增加多级 fallback
- `vite.config.js` 添加 `base: './'` 确保构建产物相对路径引用

**文档更新：**
- 新增开发日志：`docs/开发日志-2026-04-30-v08.13.md`

### v26.04.29.10.25（2026-04-29）

**新增功能：**
- 医生端：新增"快速接诊"模式，勾选后隐藏主诉/现病史/既往史/体格检查/医嘱五个非必填字段，仅保留诊断与处方，提升高峰期接诊效率
- 医生端：历史就诊记录新增"补充病历"功能，支持医生在空闲时补全快速接诊期间省略的病历信息
- 医生端：药物用法下拉选项新增"静脉滴注"
- 后端：新增 `PUT /api/doctor/visits/<id>/medical-record` 接口，用于补充更新病历字段
- 后端：就诊历史接口新增 `medical_record_incomplete` 字段，标识病历是否需要补充

**兼容性修复：**
- 后端：新增 SQLite 自动列迁移机制（`_ensure_sqlite_column`），自动补齐 visit / prescription_item / drug 表缺失的列，解决旧数据库与新代码不兼容问题
- 后端：整散装联动机制临时降级——当药品不支持拆零销售时，自动回退为整包装模式而非报错，兼容旧药品数据

**文档更新：**
- 新增开发日志：`docs/开发日志-2026-04-29-v10.25.md`，记录本次所有问题分析与解决方案
