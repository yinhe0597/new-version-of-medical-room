...
>
> **2026-07-11 数据库迁移维护说明**：历史 Alembic 链已改为 forward-only
> 安全接管模式，并通过 `data/ture.db` 只读副本升级验收；原库 105,122 行、
> 全部主键、22 个历史扩展字段及 1,227 条既有外键悬空记录均保持不变。
> SQLite 与自动化门禁已通过；真实 MySQL 8.0.26 的 `fresh/history/retry`
> 隔离验收已于 2026-07-12 全部通过。详见
> [Alembic 迁移续接记录](docs/开发日志-2026-07-11-Alembic迁移续接记录.md)。
>
> 两日整体改动、验证结果和后续事项汇总见
...
>
> **2026-07-14 本机生产验收**：使用 MySQL 8.0.26 隔离库完成最终
> `fresh/history/retry`、SQLite 105,122 行业务数据直接导入、最小权限预检、
> 逻辑备份恢复逐列对账与 EXE `live/ready` 冒烟；后端 239 项测试完成，
> 238 通过、1 项按平台跳过。本机自签证书 CN 不匹配和目标云环境验收
> 仍是正式发布边界，详见[生产环境检查与前期部署](docs/生产环境检查与前期部署-2026-07-14.md)。
...
| 🔍 [2026-07-14 生产环境检查与前期部署](docs/生产环境检查与前期部署-2026-07-14.md) | Windows 成品、SQLite 历史库、依赖安全与 MySQL 待验边界 |
...
- 历史 revision 改为幂等接管已有表、列、索引和约束；禁止不安全 downgrade 与离线 `--sql` 生成。
- SQLite 升级使用显式外层事务，失败时整体回滚并恢复外键检查；迁移前已有外键悬空记录可原样保留，但禁止新增。
- `data/ture.db` 仅通过临时副本从 `bbf28ffdb4c0` 升级到 `b6e1d8f3a2c4`，105,122 行、全部主键和 22 个历史扩展字段保持不变，原文件 SHA-256 未变化。
- 新增隔离 MySQL 验收器，覆盖空库升级、运行时超前历史结构接管和非事务 DDL 中断重试；管理员凭据只通过本机 login-path 读取。
- 后端全量测试 `91/91`、迁移定向测试 `21/21`、Python 编译、前端生产构建及差异检查均通过。

> 2026-07-12 已在本机 MySQL Community Server 8.0.26 上完成 `fresh/history/retry`
> 验收，三个场景全部通过。验收器实跑发现并修复了保留列 `usage` 未转义及 JSON
> 字面量被误解析为绑定参数两处夹具问题；随机临时数据库和账号均已清理。详见
> [Alembic 迁移续接记录](docs/开发日志-2026-07-11-Alembic迁移续接记录.md)。
