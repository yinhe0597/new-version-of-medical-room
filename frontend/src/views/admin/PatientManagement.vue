  ...
  <div class="patient-management">
    <el-card>
      ...
      <el-table :data="patientList" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column prop="class_name" label="班级" min-width="100" show-overflow-tooltip />
        <el-table-column prop="phone" label="手机号" width="120" />
        <el-table-column prop="counselor_name" label="辅导员" width="90" />
        <el-table-column label="类型" width="80">
          <template #default="scope">
            <el-tag :type="scope.row.is_temporary ? 'warning' : 'success'" size="small">
              {{ scope.row.is_temporary ? '临时' : '正式' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="90px">
        <el-form-item label="类型">
          <el-radio-group v-model="form.is_temporary">
            <el-radio :value="false">正式人员</el-radio>
            <el-radio :value="true">临时人员</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="年龄">
          <el-input-number v-model="form.age" :min="1" :max="120" controls-position="right" style="width: 100%" />
        </el-form-item>
    ...
    <el-dialog v-model="showImport" title="批量导入人员" width="520px" destroy-on-close>
      <div class="import-section">
        <el-button type="primary" @click="downloadTemplate" style="margin-bottom: 16px">下载导入模板</el-button>
        <el-upload
          class="upload-demo"
          drag
          :action="uploadUrl"
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">只能上传 CSV 文件，且需按照模板格式。必填：学号、姓名、班级、辅导员姓名。</div>
          </template>
        </el-upload>
      </div>
    </el-dialog>
...
const fetchPatients = async () => {
  loading.value = true
  try {
    const res = await request.get('/admin/patients', {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    `确定要删除 ${row.name}（${row.student_id || '无学号'}）吗？`,
    '警告',
